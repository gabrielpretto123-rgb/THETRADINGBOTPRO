...
// Database utenti (sostituisci con un database vero in futuro)
const usersDB = {};
const usersFile = './users.json';
if (fs.existsSync(usersFile)) {
  Object.assign(usersDB, JSON.parse(fs.readFileSync(usersFile)));
}
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    if (usersDB[email]) return res.status(400).json({ error: 'Email già registrata' });
    
    usersDB[email] = {
      password: await bcrypt.hash(password, 10),
      configs: {}
    };
    fs.writeFileSync(usersFile, JSON.stringify(usersDB));
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    const token = jwt.sign({ email }, 'tuo_segreto_sicuro', { expiresIn: '1h' });
    res.json({ token });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
// Lista titoli popolari
app.get('/api/popular-stocks', (req, res) => {
  const popularStocks = ['AAPL', 'MSFT', 'TSLA', 'NVDA', 'GOOGL', 'AMZN', 'META'];
...
app.post('/api/backup', async (req, res) => {
  try {
    // Implementa qui il backup su Google Drive
